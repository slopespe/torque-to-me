...
def prov(doc_id, pages, section):
    return {"document_id": doc_id, "match": "curated", "pages": pages, "manual_section": section}
...
def curate(graph) -> None:
    doc_id = next(
        (
            ...
            if isinstance(d.get("__provenance__"), dict) and d["__provenance__"].get("document_id")
        ),
        "demo",
    )
    ...
    oil_filter = add_part(
        ...
        [4],
        "ENGINE OIL FILTER REPLACEMENT (2-4)",
    )
    sealing_washer = add_part(
        "drain_plug_sealing_washer",
    ...
    filter_repl = add_procedure(
        ...
        [
            ...
            "Pour in the recommended oil to the upper level on the filler cap/dipstick.",
        ],
        [4],
        "ENGINE OIL FILTER REPLACEMENT (2-4)",
    )
    check_bolt = add_procedure(
        "crankcase_oil_check_bolt",
        "CRANKCASE OIL CHECK BOLT",
        [
            ...
            "Run the engine, stop it and wait a few minutes, then remove the oil check bolt.",
            "The crankcase oil level is correct if oil is flush with the bottom "
            "of the check bolt hole.",
            "Install the oil check bolt and recheck the level with the oil filler cap/dipstick.",
        ],
        [3, 4],
        "CRANKCASE OIL CHECK BOLT (2-3/2-4)",
    )
    ...
    for title in (
        ...
        "OIL STRAINER NUT CLEANING/INSPECTION",
        "OIL PUMP SERVICING",
    ):
        try:
...
def run(args: argparse.Namespace) -> None:
    ...
    print(
        f"Curated {graph_path}: {before[0]} -> {graph.number_of_nodes()} nodes, "
